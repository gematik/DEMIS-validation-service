...
class FormatValidatorTest {
  ...
  @Test
  void checkValidFormats() {
    assertThat(isValidJson("{\"name\":\"John\"}")).isTrue(); // true
    assertThat(isValidJson("{\"name\":\"John\"")).isFalse(); // false
    assertThat(isValidXml("<name>John</name>")).isTrue(); // true
    assertThat(isValidXml("<name>John</name")).isFalse(); // false
  }
}
